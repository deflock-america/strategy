#!/usr/bin/env python3
"""
DeFlock America FOIA Dashboard Generator - 100% BULLETPROOF
"""
...
def scan_foia_folder():
    ...
    if not os.path.exists(foia_path):
        os.makedirs(foia_path, exist_ok=True)
        return responses
...
def generate_status_table(foia_data, issues_data):
    """Generate Markdown table + RETURN totals."""
    table_lines = []
    ...
    # RETURN TABLE + TOTALS
    return '\n'.join(table_lines), total_files, total_size, total_cities
...
def send_discord_if_configured(total_files, total_size, total_cities, foia_data):
    ...
    try:
        ...
        repo_url = f"https://{REPO_NAME.split('/')[0]}.github.io/{REPO_NAME.split('/')[1]}/"
        ...
        payload = {
            "embeds": [{
                ...
                "color": 0x00ff00,
                "fields": [
                    {"name": "📊 Stats", "value": f"{total_cities} cities | {total_files} files | {total_size:.1f}MB", "inline": True},
                    {"name": "🏙️ Cities", "value": cities_str + ('...' if len(foia_data) > 3 else ''), "inline": True},
                    {"name": "🔗 Dashboard", "value": f"[Live Site]({repo_url})", "inline": False}
                ]
            }]
        }
...
if __name__ == '__main__':
    ...
    # Generate table + GET TOTALS BACK
    table_md, total_files, total_size, total_cities = generate_status_table(foia_data, issues_data)
    ...
    if success:
        ...
        print(f"📊 Totals: {total_cities} cities, {total_files} files, {total_size:.1f}MB")
        print("\n" + table_md[:500] + "...")
        
        # Discord notification WITH PROPER VARIABLES
        send_discord_if_configured(total_files, total_size, total_cities, foia_data)
    else:
